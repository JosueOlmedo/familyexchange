...
function defaultState() {
  return {
    config: { maxGifts: 5, eventName: 'Intercambio Navidad 2025', budget: '', adminPin: '', senderEmail: '', emailjsPublicKey: '', emailjsServiceId: '', emailjsTemplateId: '' },
    families: [],
    wishlists: {},
    sorteoResult: null,
    sorteoDate: null
...
// Auto-refresh from Firebase every 30s (skip if editing)
function startAutoRefresh() {
  setInterval(async () => {
    ...
    state = { ...defaultState(), ...data, config: { ...data.config, adminPin: cfg.adminPin, emailjsPublicKey: cfg.emailjsPublicKey, emailjsServiceId: cfg.emailjsServiceId, emailjsTemplateId: cfg.emailjsTemplateId, senderEmail: cfg.senderEmail } };
    saveLocal();
    renderFamilies();
    populateWishlistSelect();
  }, 30000);
...
function checkPin() {
  if (document.getElementById('pinInput').value === state.config.adminPin) {
    document.getElementById('pinOverlay').classList.add('hidden');
    showApp();
  } else {
...
function buildAvatarPicker() {
  ...
  AVATARS.forEach(emoji => {
    ...
    d.style.cssText = 'display:flex;align-items:center;justify-content:center;font-size:2rem;';
    d.textContent = emoji;
...
// ==================== THEME ====================
function initTheme() {
  const t = localStorage.getItem('intercambio_theme') || 'dark';
...
function toggleTheme() {
  ...
  localStorage.setItem('intercambio_theme', n);
  updateThemeIcon(n);
}
...
function updateLangButton() {
  ...
  if (b) b.textContent = i18n.current.toUpperCase();
  const r = document.getElementById('regLangToggle');
  if (r) r.textContent = i18n.current === 'es' ? 'ES / EN' : 'EN / ES';
}
...
function createSnowflakes() {
  ...
  for (let i = 0; i < 30; i++) {
    ...
    e.textContent = f[Math.floor(Math.random() * 4)];
    e.style.left = Math.random() * 100 + '%';
    e.style.fontSize = (Math.random() * 1.2 + 0.5) + 'rem';
...
function toast(msg, type = 'info') {
  ...
  e.textContent = msg; e.className = `toast ${type}`;
...
function getAllMembers() {
  return state.families.flatMap(f => f.members.filter(m => m.name.trim()).map(m => ({ ...m, familyId: f.id, familyName: f.name })));
}
...
function renderFamilies() {
  ...
  if (!state.families.length) {
    c.innerHTML = `<div class="card" style="text-align:center;color:var(--text-secondary);"><i class="fas fa-users" style="font-size:2rem;"></i><p>${i18n.t('families_empty')}</p></div>`;
    return;
  }
  c.innerHTML = state.families.map(f => `
    <div class="family-card"><h3><span><i class="fas fa-home"></i> ${esc(f.name)}</span>
      <button class="btn btn-danger" onclick="removeFamily('${f.id}')"><i class="fas fa-trash"></i></button></h3>
      <div id="members-${f.id}">${f.members.map(m => `
      ...
      <button class="btn btn-secondary" onclick="addMember('${f.id}')" style="margin-top:0.5rem;"><i class="fas fa-user-plus"></i> ${i18n.t('families_add_member')}</button>
    </div>`).join('');
}

async function addMember(familyId) {
  const f = state.families.find(f => f.id === familyId);
  if (!f) return;
  f.members.push({ id: uid(), name: '', email: '' });
  saveLocal();
  await CloudStorage.saveFamilies(state.families);
  renderFamilies();
  setTimeout(() => { const i = document.querySelectorAll(`#members-${familyId} .member-row:last-child input`); if (i[0]) i[0].focus(); }, 50);
}

async function updateMember(familyId, memberId, field, value) {
  const f = state.families.find(f => f.id === familyId);
  const m = f?.members.find(m => m.id === memberId);
  if (!m) return;
  if (field === 'name' && value.trim()) {
    const dup = f.members.find(x => x.id !== memberId && x.name.trim().toLowerCase() === value.trim().toLowerCase());
...
async function removeFamily(familyId) {
  const f = state.families.find(f => f.id === familyId);
  if (!f || !confirm(i18n.t('families_confirm_delete').replace('{name}', f.name))) return;
...
async function resetPassword(familyId, memberId) {
  const f = state.families.find(f => f.id === familyId);
  const m = f?.members.find(m => m.id === memberId);
  ...
  saveLocal();
  await CloudStorage.saveFamilies(state.families);
  toast(`🔑 Password reset: ${m.name}`, 'success');
}

// ==================== WISHLISTS ====================
function populateWishlistSelect() {
  const s = document.getElementById('wishlistPerson');
  s.innerHTML = `<option value="">${i18n.t('wishlists_select')}</option>` +
    state.families.map(f => {
      const o = f.members.filter(m => m.name.trim()).map(m => `<option value="${m.id}">${esc(m.name)}</option>`).join('');
      return o ? `<optgroup label="${esc(f.name)}">${o}</optgroup>` : '';
    }).join('');
...
function renderWishlistItems(memberId) {
  const items = state.wishlists[memberId] || [], c = document.getElementById('wishlistItems'), max = state.config.maxGifts;
  ...
  c.innerHTML = items.map((item, i) => `
    <div class="wish-item"><h4><span>${i18n.t('wishlists_gift_n').replace('{n}', i+1)}</span>
      <button class="btn btn-danger" onclick="removeWishItem('${memberId}',${i})"><i class="fas fa-trash"></i></button></h4>
      <div class="form-group"><label>${i18n.t('wishlists_gift_title')}</label><input type="text" value="${esc(item.title)}" onchange="updateWishItem('${memberId}',${i},'title',this.value)"></div>
      <div class="form-group"><label>${i18n.t('wishlists_gift_link')}</label><input type="url" value="${esc(item.link)}" onchange="updateWishItem('${memberId}',${i},'link',this.value)" placeholder="https://..."></div>
      <div class="form-group"><label>${i18n.t('wishlists_gift_image')}</label><input type="url" value="${esc(item.imageUrl)}" onchange="updateWishItem('${memberId}',${i},'imageUrl',this.value); this.nextElementSibling.src=this.value;" placeholder="https://..."><img class="preview-img" src="${esc(item.imageUrl)}" onerror="this.style.display='none'" onload="this.style.display='block'" style="${item.imageUrl?'':'display:none'}"></div>
      <div class="form-group"><label>${i18n.t('wishlists_gift_desc')}</label><textarea onchange="updateWishItem('${memberId}',${i},'description',this.value)">${esc(item.description)}</textarea></div>
      <div class="form-group"><label>${i18n.t('wishlists_gift_notes')}</label><textarea onchange="updateWishItem('${memberId}',${i},'notes',this.value)">${esc(item.notes)}</textarea></div>
    </div>`).join('');
}
...
function addWishItem() {
  ...
  if (state.wishlists[id].length >= state.config.maxGifts) return toast(i18n.t('wishlists_max').replace('{n}', state.config.maxGifts), 'error');
  state.wishlists[id].push({ title: '', link: '', imageUrl: '', description: '', notes: '' });
  saveLocal();
  renderWishlistItems(id);
}

function updateWishItem(memberId, i, field, value) {
  if (state.wishlists[memberId]?.[i]) { state.wishlists[memberId][i][field] = value; saveLocal(); }
}

function removeWishItem(memberId, i) {
  state.wishlists[memberId]?.splice(i, 1); saveLocal(); renderWishlistItems(memberId);
}
...
function renderMemberUrls() {
  ...
  c.innerHTML = members.map(m => {
    const url = `${base}/my-list.html?id=${m.id}`;
    return `<div class="url-row"><span class="name">${esc(m.name)}</span>
      <input type="text" value="${url}" readonly onclick="this.select()">
      <button class="btn btn-sm btn-gold" onclick="copyUrl(this,'${url}')"><i class="fas fa-copy"></i> ${i18n.t('copy')}</button></div>`;
  }).join('');
}

function copyUrl(btn, url) {
  navigator.clipboard.writeText(url).then(() => {
    const o = btn.innerHTML; btn.innerHTML = `<i class="fas fa-check"></i> ${i18n.t('copied')}`;
...
function checkSorteoReady() {
  ...
  const el = document.getElementById('sorteoStatus'), btn = document.getElementById('startSorteo');
  const issues = [];
  ...
  if (issues.length) {
    el.innerHTML = issues.map(i => `<p class="warning"><i class="fas fa-exclamation-triangle"></i> ${i}</p>`).join('');
    btn.disabled = true;
  } else {
    el.innerHTML = `<p class="ready"><i class="fas fa-check-circle"></i> ${i18n.t('sorteo_ready').replace('{n}', members.length).replace('{f}', families.length)}</p>`;
    btn.disabled = false;
  }
...
function performSorteo(members) {
  const fm = {};
  state.families.forEach(f => f.members.forEach(m => { fm[m.id] = f.id; }));
  for (let t = 0; t < 1000; t++) {
    const r = [...members]; shuffleArray(r); let ok = true; const res = [];
    for (let i = 0; i < members.length; i++) {
      if (members[i].id === r[i].id || fm[members[i].id] === fm[r[i].id]) { ok = false; break; }
      res.push({ giverId: members[i].id, receiverId: r[i].id });
    }
    if (ok) return res;
  }
  return null;
...
async function startSorteo() {
  ...
  document.getElementById('rouletteContainer').classList.add('active');
...
function buildRouletteWheel(members) {
  ...
  members.forEach((m, i) => {
    const s = document.createElement('div'); s.className = 'roulette-segment';
    s.style.background = colors[i%8]; s.style.setProperty('--half-angle',(angle/2)+'deg');
    s.style.transform = `rotate(${angle*i}deg)`;
    s.style.clipPath = `polygon(0% 100%,100% 100%,${50+50*Math.cos((angle-90)*Math.PI/180)}% ${50+50*Math.sin((angle-90)*Math.PI/180)}%)`;
    s.style.cssText += 'width:100%;height:100%;top:0;left:0;transform-origin:50% 50%;';
    const sp = document.createElement('span'); sp.textContent = m.name.length > 10 ? m.name.substring(0,10)+'…' : m.name;
    s.appendChild(sp); w.appendChild(s);
  });
}

function showResults(result) {
  const members = getAllMembers(), map = Object.fromEntries(members.map(m=>[m.id,m]));
  const c = document.getElementById('sorteoResults'); c.classList.remove('hidden');
  c.innerHTML = `<h3 style="margin-bottom:1rem;color:var(--green);"><i class="fas fa-check-circle"></i> ${i18n.t('sorteo_results')}</h3>` +
    result.map(r => { const g=map[r.giverId],rv=map[r.receiverId]; return (!g||!rv)?'':`<div class="result-pair"><span class="person">${esc(g.name)}</span><span class="arrow"><i class="fas fa-arrow-right"></i></span><span class="person">${esc(rv.name)}</span><span style="color:var(--text-secondary);font-size:0.8rem;margin-left:auto;">(${esc(rv.familyName)})</span></div>`; }).join('') +
...
async function clearSorteo() {
  ...
  state.sorteoResult = null; state.sorteoDate = null; saveLocal();
  await CloudStorage.clearSorteo();
  ...
  const w = document.getElementById('rouletteWheel'); w.style.transition='none'; w.style.transform='rotate(0deg)';
  checkSorteoReady();
}

// ==================== EMAIL ====================
function buildEmailHtml(giverName, receiverName, wishlistText, eventName, budget) {
  return `<div style="font-family:'Segoe UI',system-ui,sans-serif;max-width:600px;margin:0 auto;border-radius:16px;overflow:hidden"><div style="background:linear-gradient(135deg,#c0392b,#922b21);padding:30px 20px;text-align:center"><div style="font-size:48px">🎄🎁🎄</div><h1 style="color:#fff;font-size:24px;margin:10px 0 5px">${esc(eventName)}</h1></div><div style="padding:30px 25px;background:#fff"><p style="font-size:16px;color:#2c3e50">${i18n.t('email_greeting')} <strong>${esc(giverName)}</strong> 👋</p><p style="font-size:15px;color:#555;line-height:1.6">${i18n.t('email_intro')}</p><div style="background:linear-gradient(135deg,#27ae60,#1e8449);border-radius:12px;padding:20px;text-align:center;margin:20px 0"><div style="font-size:36px">🎁</div><p style="color:rgba(255,255,255,0.8);font-size:13px;margin:5px 0">${i18n.t('email_you_got')}</p><h2 style="color:#fff;font-size:26px;margin:5px 0">${esc(receiverName)}</h2></div><div style="background:#fef9e7;border:2px solid #f39c12;border-radius:10px;padding:12px 16px;margin:15px 0;text-align:center"><span style="font-size:14px;color:#f39c12;font-weight:700">💰 ${i18n.t('email_budget_label')}: ${esc(budget)}</span></div><div style="margin-top:20px"><h3 style="color:#c0392b;font-size:16px;border-bottom:2px solid #ecf0f1;padding-bottom:8px">🎁 ${i18n.t('email_wishlist_title')}</h3><pre style="font-family:'Segoe UI',system-ui,sans-serif;font-size:14px;color:#555;line-height:1.8;white-space:pre-wrap;background:#f8f9fa;border-radius:8px;padding:15px;margin-top:10px">${esc(wishlistText)}</pre></div><p style="font-size:13px;color:#999;margin-top:25px;text-align:center">${i18n.t('email_secret')}</p></div><div style="background:#1a1a2e;padding:20px;text-align:center"><p style="color:#666;font-size:12px;margin:0">❄️ ${i18n.t('email_footer')} ${esc(eventName)} ❄️</p></div></div>`;
}
...
async function sendEmails() {
  ...
  try { emailjs.init(emailjsPublicKey); } catch(e) { toast('EmailJS error: '+e.message,'error'); toggleModal(false); return; }
  const members = getAllMembers(), map = Object.fromEntries(members.map(m=>[m.id,m]));
  const btn = document.getElementById('modalSendEmail'); btn.disabled = true;
  btn.innerHTML = `<i class="fas fa-spinner fa-spin"></i> ${i18n.t('sorteo_sending')}`;
  let sent=0, errors=0;
  for (const pair of state.sorteoResult) {
    const g=map[pair.giverId], rv=map[pair.receiverId];
    if (!g?.email||!rv) { errors++; continue; }
    const wt = (state.wishlists[rv.id]||[]).filter(w=>w.title.trim()).map((w,i)=>{ let l=`${i+1}. ${w.title}`; if(w.description)l+=` - ${w.description}`; if(w.link)l+=` | Link: ${w.link}`; if(w.notes)l+=` | ${w.notes}`; return l; }).join('\n') || i18n.t('email_no_wishlist');
    try { await emailjs.send(emailjsServiceId,emailjsTemplateId,{to_email:g.email,email_subject:i18n.t('email_subject').replace('{event}',eventName),email_body:buildEmailHtml(g.name,rv.name,wt,eventName,budget||'N/A')}); sent++; }
    catch(e) { console.error(`Email error (${g.name}):`,e); errors++; }
  }
...
async function syncToCloud() {
  ...
  toast(ok?'✅ Uploaded':'❌ Failed', ok?'success':'error');
}

async function syncFromCloud() {
  toast('⏳ Downloading...','info');
  const data = await CloudStorage.load();
  if (!data) { toast('❌ Failed','error'); return; }
  const cfg = { ...state.config };
  state = { ...defaultState(), ...data, config: { ...(data.config||{}), adminPin:cfg.adminPin, emailjsPublicKey:cfg.emailjsPublicKey, emailjsServiceId:cfg.emailjsServiceId, emailjsTemplateId:cfg.emailjsTemplateId, senderEmail:cfg.senderEmail } };
  saveLocal(); loadConfigUI(); renderFamilies(); populateWishlistSelect();
...
function exportExcel(type) {
  ...
  if (type==='wishlists'||type==='all') { const d=[]; members.forEach(m=>(state.wishlists[m.id]||[]).forEach((w,i)=>d.push({Name:m.name,Family:m.familyName,'#':i+1,Gift:w.title,Link:w.link,Image:w.imageUrl,Description:w.description,Notes:w.notes}))); if(!d.length)d.push({Name:'No wishlists'}); XLSX.utils.book_append_sheet(wb,XLSX.utils.json_to_sheet(d),'Wishlists'); }
  if (type==='sorteo'||type==='all') { const d=(state.sorteoResult||[]).map(r=>{const g=map[r.giverId],rv=map[r.receiverId];return{Giver:g?.name||'?','Giver Family':g?.familyName||'?','Giver Email':g?.email||'?',Receiver:rv?.name||'?','Receiver Family':rv?.familyName||'?'};}); if(!d.length)d.push({Giver:'No draw'}); XLSX.utils.book_append_sheet(wb,XLSX.utils.json_to_sheet(d),'Draw'); }
  const fn=`${state.config.eventName.replace(/[^a-zA-Z0-9]/g,'_')}_${type}.xlsx`; XLSX.writeFile(wb,fn); toast(`✅ ${fn}`,'success');
}
